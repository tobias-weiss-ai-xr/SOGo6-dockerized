...
test.describe('Endpoint mutation matrix — Jobs (3 write endpoints / 3 tests)', () => {
  test('MUT-jobs-1: authenticated POST /jobs/0 does not 5xx', async ({ request }) => {
    const res = await request.fetch(`${REMOTE_API}/jobs/0`, {
      method: "POST",
      headers: { Authorization: `Bearer ${USER_TOKEN}`, 'Content-Type': 'application/json' },
      data: {},
    });
    test.info().annotations.push({ type: 'mutate', description: `POST /jobs/0 -> ${res.status()}` });
    expect(OK_STATUSES, `auth write POST /jobs/0 -> ${res.status()}`).toContain(res.status());
  });

  test('MUT-jobs-2: authenticated POST /jobs/0/cancel does not 5xx', async ({ request }) => {
  ...
  test('MUT-jobs-3: authenticated POST /jobs/0/result does not 5xx', async ({ request }) => {
    const res = await request.fetch(`${REMOTE_API}/jobs/0/result`, {
      method: "POST",
      headers: { Authorization: `Bearer ${USER_TOKEN}`, 'Content-Type': 'application/json' },
      data: {},
    });
    test.info().annotations.push({ type: 'mutate', description: `POST /jobs/0/result -> ${res.status()}` });
    expect(OK_STATUSES, `auth write POST /jobs/0/result -> ${res.status()}`).toContain(res.status());
  });
});
